...
def get_bot_response():
    ...
    if a == "chao_hoi":
        ...
        return jsonify({"action": "print", "message":"Bạn vui lòng nhập tòa,tầng,số phòng và thời điểm muốn xem báo cáo"})
    elif a== "user_list":
        ...
        building,floor,room,type_id,da_te,fr_om,t_o=extract_info(userText)
        print(da_te)
        if building is None and (floor is not None or room is not None or type_id is not None):
            return jsonify({"action":"print", "message":"Không tìm thông tin nào hợp lệ, vui lòng nhập lại(ví dụ: "
                                                        "tòa A4 tầng 3 phòng 10 thiết bị A504 ngày 25/07/2020 từ 12:00:00 đến "
                                                        "14:00:00 )"})
            ...
            return jsonify({"action":"print", "message":"Không tìm thấy tầng nhà nào hợp lệ, vui lòng nhập thêm(ví dụ: "
                                                        "tòa A4 tầng 3 phòng 10 thiết bị A504 ngày 25/07/2020 từ 12:00:00 đến "
                                                        "14:00:00 )"})
        elif room is None and type_id is not None:
            return jsonify(
                {"action": "print", "message": "Không tìm thấy thiết bị nào hợp lệ, vui lòng nhập lại(ví dụ: "
                                               "tòa A4 tầng 3 phòng 10 thiết bị A504 ngày 25/07/2020 từ 12:00:00 đến "
                                               "14:00:00 )"})
            ...
            return return_report(building, floor, room,type_id,da_te, fr_om, t_o)
    elif a == "kw":
...
def return_electric_useage(a):
    if a == "hôm nay" or a=="ngày hôm nay":
        ...
        return jsonify({"action": "Electric_Consumption","from": str(midnight), "to": str(end_of_day)})
def return_report(building, floor, room,type_id,da_te,fr_om,t_o):
    if da_te == "hôm nay":
        print(1)
        dt = date.today()
        midnight = datetime.combine(dt, datetime.min.time()).timestamp()
        end_of_day = datetime.combine(dt, datetime.max.time()).timestamp()
        return jsonify(
            {"action": "report_detail", "building": building, "floor": floor, "room": room,"type":type_id,"from": str(midnight),
             "to": str(end_of_day)})
    elif da_te == "bây giờ" or da_te == "hiện tại":
        print(2)
        ts = time.time()
        return jsonify(
            {"action": "report_detail", "building": building, "floor": floor, "room": room,"type":type_id, "from": str(ts),
             "to": str(ts)})
    elif da_te == "hôm qua":
        print(3)
        dt = date.today() - timedelta(days=1)
        ...
        print(4)
        return jsonify(
            {"action": "report_detail", "building": building, "floor": floor, "room": room,"type":type_id, "from": None,
             "to": None})
        ...
        end_of_day = datetime.combine(c, datetime.max.time()).timestamp()
        return jsonify(
            {"action": "report_detail", "building": building, "floor": floor, "room": room, "type": type_id,
...
def extract_info(b):
    ...
    if date is None:
        ...
        t_o = get_input(b, "đến")
    return building, floor, room,type_id, date, fr_om, t_o
