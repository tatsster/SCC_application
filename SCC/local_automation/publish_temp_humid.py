...
def publish_fake_sensor_values_to_mqtt():
    ...
    try:
        ...
        temperature_value = float("{0:.1f}".format(temperature_value + random.choice([0,step_change_value])*random.random()))
        humidity_value = float("{0:.1f}".format(humidity_value + random.choice([0,step_change_value])*random.random()))

        while temperature_value > 37.0 or temperature_value < 16.0:
            temperature_value = 30.0 + random.choice([0,step_change_value])*random.random()
            humidity_value = 60.0 + random.choice([0,step_change_value])*random.random()

        data = {}
        data['device_id'] = "TEMP-HUD100"
